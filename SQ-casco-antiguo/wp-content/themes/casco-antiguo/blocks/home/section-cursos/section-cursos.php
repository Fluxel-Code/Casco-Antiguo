...
$sDescription = "HOME - sección de nuestros cursos";
// Ej: casco-antiguo-home
$sCategory = "casco-antiguo-home";

// Ej: array('home','SQ','Cabecera','Squembri');
$aKW = array('home','SQ','Cursos','Squembri');
// Ej: array("post", "page")
$aPostType = array("post", "page");

$sCallBack = "FUN_BLOCK_HOME_Cursos";

$sRutaStyle = get_template_directory_uri()."/blocks/home/section-cursos/assets/css/style.css";

sq_blocks::Blocks_enqueue($sBlocName,$sLanding,$sDescription,$aKW,$aPostType,$sRutaStyle,$sCategory,$sCallBack);


function FUN_BLOCK_HOME_Cursos($block, $content = '', $is_preview = false, $post_id = 0){
    if ($is_preview) {
        
    } 
    // Establecer el id de la seccion para el bloque, debe ser único
    $sIdBlock = "HOME_CURSOS";
    FUN_BLOCK_HOME_SectionCursos____Base($sIdBlock);
}

function FUN_BLOCK_HOME_SectionCursos____Base($sIdBlock){
    // AJUSTES AUTOMÁTICOS!!!!!!!
    /**
     * 
    ...
    <section class="<?php echo $sClass; ?>" id="<?php echo $sIdBlock; ?>" >
        <div class="contenedor <?php echo $sContenedor; ?>" <?php echo $sCustomBox; ?>>
            <?php FUN_BLOCK_HOME_SectionCursos____content(); ?>
        </div>
